...
namespace Erethan.ScriptableServices.Pool
{
	/// <summary>
	...
	public class ComponentPool<T> : Pool<T> where T : Component
	{
		...
		private Transform PoolRoot
		{
			get
			{
				if (_poolRoot == null)
				{
					_poolRoot = new GameObject(nameof(ComponentPool<T>)).transform;
		...
		protected override T Create()
		{
			...
			if (Prefab == null)
            {
				newMember = new GameObject().AddComponent<T>();
            }
			else
            {
				newMember = Object.Instantiate(Prefab);
            }
